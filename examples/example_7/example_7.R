...
source("example_7_helper.R")

my_html <- create_html() %>%
  add_js_library("plotly") %>%
  add_title("An illustration of non-parametric density estimation") %>%
  add_button(value = "add 1 data points", onclick = "simulate_data(1)") %>%
  add_button(value = "add 10 data points", onclick = "simulate_data(10)") %>%
  add_button(value = "add 100 data points", onclick = "simulate_data(100)") %>%
  add_button(value = "add 1000 data points", onclick = "simulate_data(1000)") %>%
  add_button(value = "play / pause", onclick = "play_pause()") %>%
...
r_fun <- function(msg) {
  # Estimate density
  data1 <<- c(data1, simple_DGP(msg$num_sim))  # Hack!
