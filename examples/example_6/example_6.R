# Example 6. Get familiar with the common visual tasks in JS (p5.js) and
...
my_html <- create_html() %>%
  ...
  add_container(id = "row_1") %>%
    ...
    add_container(id = "column_2", into = "row_1") %>%
      ...
      add_button(into = "column_2", value = "Clear", onclick = "plot_grid()")
...
my_html %<>%
  add_script_from_file("example_6_helper.js") %>% 
...
load("example_6_fitted_glmnet", verbose = T)
r_fun <- function(msg) {
...
start_app(my_html, r_fun, T, assets_folder = ".")
